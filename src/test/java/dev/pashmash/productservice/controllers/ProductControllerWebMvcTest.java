...
public class ProductControllerWebMvcTest {
    ...
    void createProductShouldCreateANewProduct() throws Exception {
        ...
        mockMvc.perform(post("/products").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(productToCreate))).andExpect(content().string(objectMapper.writeValueAsString(expectedProduct))).andExpect(status().is(200));
//                .andExpect((ResultMatcher) jsonPath("$.id", matches("1001"))); // this is bad idea use something like hamcrest here
    }
}
